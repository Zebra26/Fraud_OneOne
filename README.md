...
2. Launch stack: `docker compose -f docker-compose.dev.yml up --build`
   - Alternative (Windows/PowerShell): `./scripts/dev_up.ps1` après `./scripts/bootstrap.ps1`
...
See also `DEVELOPMENT.md` for a concise dev workflow.
...
- Send a few synthetic requests (JWT + HMAC generated by the tool):
...
- Backend fast path (no network hop):
  - `USE_LOCAL_INFERENCE=true` makes the backend compute the score using `AdvancedFraudModelService`.
- Benchmark mode (avoid storage overhead):
  - `PERF_MODE=true` disables Mongo/Redis writes on the score path and relaxes checks that require Redis (JTI replay, per-device limiter). JWT+HMAC verification remains enforced.
- Workers:
  - Backend: `USE_GUNICORN=true`, `GUNICORN_WORKERS`, `GUNICORN_TIMEOUT`.
  - Inference: Uvicorn workers via `UVICORN_WORKERS`.

## ONNX Acceleration (Inference)

- Enable runtime acceleration:
  - Set `USE_ONNXRUNTIME=true` for the `ml-inference` service (see `docker-compose.dev.yml`).
- Supervised model export:
  ...
  - Optional micro-batching: `INFER_ONNX_BATCH=true`, `INFER_BATCH_SIZE`, `INFER_BATCH_TIMEOUT_MS`.
